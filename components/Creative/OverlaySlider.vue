<template>
  <div class="relative w-3/4">
    <UITitleCenter class="pb-6">Image Overlay Slider</UITitleCenter>

    <div class="h-128 important">
      <div class='parent'>
        <div class='slider'>
          <button type="button" id='right' class='right' name="button" @click="rightSlide">
       ...
       <g>
        <path style='fill: #9d9d9d;' d="M360.731,229.075l-225.1-225.1c-5.3-5.3-13.8-5.3-19.1,0s-5.3,13.8,0,19.1l215.5,215.5l-215.5,215.5
          ...
          <button type="button" id='left' class='left' name="button" @click="leftSlide">
...
export default {
  ...
  methods: {
    leftSlide() {
      if (this.click) {
        if (this.curpage == 1) this.curpage = 5;
        this.sliding = true;
        this.curpage--;
        this.svg = true;
        this.click = false;
        for (let k = 1; k <= 4; k++) {
          var a1 = document.getElementById(this.pagePrefix + k);
          a1.className += " tran";
        }
        setTimeout(() => {
          this.move();
    ...
    move() {
      if (this.sliding) {
        ...
        if (this.svg) {
          for (let j = 1; j <= 9; j++) {
            var c = document.getElementById(this.transitionPrefix + j);
        ...
        setTimeout(() => {
          for (let i = 1; i <= 4; i++) {
            if (i == this.curpage) {
              var a = document.getElementById(this.pagePrefix + i);
              a.className += " up1";
            } else {
              var b = document.getElementById(this.pagePrefix + i);
        ...
        setTimeout(() => {
          if (this.svg) {
            for (let j = 1; j <= 9; j++) {
              ...
              c.setAttribute("class", this.transitionPrefix + j + " steap");
            }
          } else {
            for (let j = 10; j <= 18; j++) {
              var c = document.getElementById(this.transitionPrefix + j);
